...
def get_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output", type=str, default="./", help="The path at which to save the dataset"
    )
    parser.add_argument(
        "--distractors",
        type=int,
    ...
    parser.add_argument(
        ...
        type=str,
        default="pdf",
        help="The type of the document, must be one of the accepted doctypes",
        choices=["pdf", "txt", "json", "api"],
    )
    parser.add_argument(
        "--openai_key",
        type=str,
...
def get_chunks(
    ...
    if doctype == "api":
        ...
        text_splitter = SemanticChunker(
            OpenAIEmbeddings(openai_api_key=OPENAPI_API_KEY),
...
def generate_label(question: str, context: Any, doctype: DocType = "pdf") -> str | None:
    ...
    """
    question = (
        encode_question(question, context)
        if doctype == "api"
        else encode_question_gen(question, context)
    )
    response = client.chat.completions.create(
...
def add_chunk_to_dataset(
    ...
    qs = (
        generate_instructions(chunk, x)
        if doctype == "api"
        else generate_instructions_gen(chunk, x)
    )
    for q in qs:
        datapt = {
            "id": None,
